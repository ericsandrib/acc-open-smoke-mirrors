...
export function collectJourneySupportingDocumentRows(state: WorkflowState): JourneySupportingDocRow[] {
  ...
  for (const t of getAllOpenAccountsTasks(state)) {
    ...
    pushUnique(rowsFromTaskData(state, getOpenAccountsTaskData(state, t.id), scope, labelMap))
  }
  ...
  if (state.activeChildActionId) {
    ...
    if (child) {
      ...
      pushUnique(rowsFromTaskData(state, childRoot, child.name, labelMap))
      ...
      for (const st of visible) {
        ...
        pushUnique(rowsFromTaskData(state, subData, scope, labelMap))
      }
